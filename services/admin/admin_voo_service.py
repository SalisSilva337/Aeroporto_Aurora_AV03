...
def alterar_voo_service(id_voo, id_aviao, local_partida, local_destino, duracao):
    try:
        conn = criar_conexao()
        cursor = conn.cursor()
        query = "UPDATE id_voo, id_aviao, local_partida, local_destino, duracao FROM voo ORDER BY id_voo"
        cursor.execute(query,(id_voo, id_aviao, local_partida, local_destino, duracao))
        conn.commit()
        print("as informacoes do Voo foram alteradas com sucesso")
    except Exception as e:
        print(f"erro ao alterar informarcoes do voo: {e}")
    finally:
        cursor.close()
        conn.close()
